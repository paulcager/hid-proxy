...
1. **ESP32 #1 GPIO17** → **ESP32 #2 GPIO18** (TX → RX)
2. **ESP32 #1 GPIO18** → **ESP32 #2 GPIO17** (RX → TX)
...
|-----|----------|-----------|-------------|
| GPIO2/3 | USB Host D+/D- | Varies | USB OTG cable (keyboard) |
| GPIO17 | UART TX | Output | ESP32 #2 GPIO18 |
| GPIO18 | UART RX | Input | ESP32 #2 GPIO17 |
...
| GPIO17 | UART TX | Output | ESP32 #1 GPIO18 |
| GPIO18 | UART RX | Input | ESP32 #1 GPIO17 |
...
```
ESP32 #1           ESP32 #2
GPIO17 (TX) ────> GPIO17 (TX)  ← NO! Both transmitting
GPIO18 (RX) <──── GPIO18 (RX)  ← NO! Both receiving
```

### ✅ CORRECT: TX → RX, RX → TX
```
ESP32 #1           ESP32 #2
GPIO17 (TX) ────> GPIO18 (RX)  ✓ Data flows
GPIO18 (RX) <──── GPIO17 (TX)  ✓ Data flows
```
...
2. Touch ESP32 #1 GPIO17 and ESP32 #2 GPIO18 → Should beep
3. Touch ESP32 #1 GPIO18 and ESP32 #2 GPIO17 → Should beep
4. Touch ESP32 #1 GND and ESP32 #2 GND → Should beep
5. Touch GPIO17 and GPIO18 on same board → Should NOT beep
...
2. Black probe on GND, red probe on GPIO17 → Should read ~3.3V (idle high)
3. Black probe on GND, red probe on GPIO18 → Should read ~3.3V (idle high)
...
3. **Try loopback test** on each board individually:
   - Wire GPIO17 → GPIO18 on same board
