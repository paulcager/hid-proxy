...
**Online mode:**
```bash
# First: Press both shifts + HOME on physical keyboard to enable web access
...
**Implementation:**
```c
// Endpoints (all require physical unlock via both-shifts+HOME):
...
// Press both shifts + HOME to enable web access for 5 minutes
```
...
# 1. Press both shifts + HOME on keyboard (enables web access for 5 min)
# 2. Then use HTTP:
...
> web unlock          # Alternative to physical both-shifts+HOME
```
...
- **Decrypted macros must be protected** - use one of:
  1. **Physical unlock required** (Recommended): HTTP endpoints only work after **both shifts + HOME** keystroke on physical keyboard. Timeout after 5 minutes of inactivity.
...
- Press **both shifts + HOME** (all at same time): Enable web access for 5 minutes
- Similar to existing both shifts + PAUSE (bootloader mode)
...
// In keyboard report handler (similar to shift+shift+PAUSE detection):
if (both_shifts_pressed(report) && report->keycode[0] == HID_KEY_HOME) {
